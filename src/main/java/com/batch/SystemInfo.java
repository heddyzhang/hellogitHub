...
public interface SystemInfo {
	...
	public final String QUERY3_04		= "INSERT INTO CMSNPTBL (RECID, ISN, KY1, KY2, KY3, KY4, HK, HI, FT, TL, SCD_KY, SLTCD)"
											+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	// 蓄積確認件数取得(国内)クエリ
	public final String QUERY1_05		= "SELECT COUNT(*) FROM (SELECT ISN FROM CMSJPTBL WHERE SCD_KY = ? AND RECID > ? UNION ALL SELECT ISN FROM CMSJPTBL WHERE SCD_KZ = ? AND RECID > ?)";
	// 蓄積確認件数取得(国外)クエリ
	public final String QUERY2_05		= "SELECT COUNT(*) FROM CMSFPTBL WHERE SCD_KY = ? AND RECID > ?";
	// 蓄積確認件数取得(非特許)クエリ
	public final String QUERY3_05		= "SELECT COUNT(*) FROM CMSNPTBL WHERE SCD_KY = ? AND RECID > ?";

	// 蓄積確認上位10件取得(国内)クエリ
	public final String QUERY1_06		= "SELECT ISN, DNO FROM (SELECT ISN, KY AS DNO FROM CMSJPTBL WHERE SCD_KY = ? AND RECID > ? UNION ALL SELECT ISN, KZ AS DNO FROM CMSJPTBL WHERE SCD_KZ = ? AND RECID > ?) T ORDER BY DNO LIMIT 10";
	// 蓄積確認上位10件取得(国外)クエリ
	public final String QUERY2_06		= "SELECT ISN, KY3 AS DNO FROM CMSFPTBL WHERE SCD_KY = ? AND RECID > ? ORDER BY DNO LIMIT 10";
	// 蓄積確認上位10件取得(非特許)クエリ
	public final String QUERY3_06		= "SELECT ISN, CASE SCD_KY WHEN 'N10' THEN KY3 ELSE KY3 || KY4 END AS DNO, CASE KY4 WHEN '   ' THEN 0 ELSE CAST(KY4 AS INTEGER) END AS IKY4 FROM CMSNPTBL WHERE SCD_KY = ? AND RECID > ? ORDER BY KY3, IKY4 LIMIT 10";

	// 蓄積確認下位5件(降順)取得(国内)クエリ
	public final String QUERY1_07		= "SELECT ISN, DNO FROM (SELECT ISN, KY AS DNO FROM CMSJPTBL WHERE SCD_KY = ? AND RECID > ? UNION ALL SELECT ISN, KZ AS DNO FROM CMSJPTBL WHERE SCD_KZ = ? AND RECID > ?) T ORDER BY DNO DESC LIMIT 5";
	// 蓄積確認下位5件(降順)取得(国外)クエリ
	public final String QUERY2_07		= "SELECT ISN, KY3 AS DNO FROM CMSFPTBL WHERE SCD_KY = ? AND RECID > ? ORDER BY DNO DESC LIMIT 5";
	// 蓄積確認下位5件(降順)取得(非特許)クエリ
	public final String QUERY3_07		= "SELECT ISN, CASE SCD_KY WHEN 'N10' THEN KY3 ELSE KY3 || KY4 END AS DNO, CASE KY4 WHEN '   ' THEN 0 ELSE CAST(KY4 AS INTEGER) END AS IKY4 FROM CMSNPTBL WHERE SCD_KY = ? AND RECID > ? ORDER BY KY3 DESC, IKY4 DESC LIMIT 5";
	...
	// 文蓄情報抽出(外国)
	public final String QUERY2			= "SELECT SCD_KY, MIN(KY3), MAX(KY3), MAX(HK), COUNT(*) FROM CMSFPTBL WHERE SLTCD = '0' GROUP BY SCD_KY WITHOUT LOCK NOWAIT";
	// 文蓄情報抽出(非特許)①
	public final String QUERY3_1			= "SELECT SCD_KY, MIN(KY3), MAX(KY3), MAX(HK), COUNT(*) FROM CMSNPTBL WHERE SLTCD = '0' GROUP BY SCD_KY WITHOUT LOCK NOWAIT";
	// 文献情報抽出(非特許)②
