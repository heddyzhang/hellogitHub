...
public class CP5P130 {
	...
	public static void main(String[] args) {
		...
		DbExtract objDbExtract3 = null;
		DbExtract objDbExtract4 = null;
		...
		try {
			...
			// 文蓄情報抽出(外国)
			objDbExtract3          = new DbExtract(SystemInfo.TBLNAME2);
			// 文蓄情報抽出(非特許)
			objDbExtract4          = new DbExtract(SystemInfo.TBLNAME3);
			...
			objDbExtract3.select(objCon, SystemInfo.QUERY2);

			// 共通検索一覧表示用情報(非特許)管理DBを種別コード(文献番号)毎にDB検索を行う
			objDbExtract4.select(objCon, SystemInfo.QUERY3_1);

			// 代表文献番号(記事番号)の蓄積先頭番号取得(非特許用)
			objDbExtract4.selectKy4Min(objCon, SystemInfo.QUERY3_2);

			// 代表文献番号(記事番号)の蓄積最終番号取得(非特許用)
			objDbExtract4.selectKy4Max(objCon, SystemInfo.QUERY3_3);

			// 文蓄情報設定
			objRangeInfo.setRangeInfo(objDbExtract1, objDbExtract2, objDbExtract3, objDbExtract4);
